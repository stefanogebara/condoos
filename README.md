...
Design language: **claymorphism + glassmorphism**. Muted sage / dusty peach / cream palette. Frosted glass cards layered over rich dusk-landscape backdrops and soft 3D clay illustrations — all generated by AI. Type set in **Inter Tight + Inter** — the modern UI pairing shared by Google Material 3 and Claude.ai. Live reference at [`/design`](docs/screenshots/09-design-system-full.png).

![CondoOS landing](docs/screenshots/01-landing.png)
...
- **Realtime** — Socket.io for live vote counts and new-suggestion toasts (stub in place).
- **Mobile** — the layout already works on small screens; a trimmed navigation would polish it.
- **Multi-tenant** — every table carries `condominium_id`; a condo picker + signup flow would unlock this.
- **Cron** — amenity reminders, weekly resident-digest emails (AI-composed from the week's activity).
