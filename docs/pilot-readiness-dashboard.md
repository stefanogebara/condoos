...
- Management-company portfolio mode is still early. Agency admins can manage private activation codes, invite or scope staff accounts to buildings, see permission review and pilot-readiness flags, act on a prioritized portfolio attention queue, compare per-building operational health scorecards, download monthly Markdown/PDF agency reports with board-ready tables, and export key operational CSVs. Server-side capability checks, frontend board navigation, and export-family limits now respect scoped agency lanes, but richer multi-building reporting still needs hardening.
